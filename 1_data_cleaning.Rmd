...
patient_ehr = enc_demo_60 %>% 
  ...
  # filter those age at last visit 60-90
  filter(age_last_visit >= 60 & age_last_visit < 90)
dim(patient_ehr) # dim = (25229,8)
```
...
dim(enc_demo_filtered) # dim = (3550721,8)
# Map to phecodes
...
dim(enc_phecode_map) # dim = (3634136,10)
...
dim(enc_phecode_map_first) # dim = (2225897,8)
...
dem_control_info = patient_ehr %>% 
  ...
  filter(age_last_visit >= 60 & age_last_visit < 90)
dim(dem_control_info) # dim = (24072,11)
```

```{r}
# combine cases and controls
dem_patients_full = rbind(dem_case_info, dem_control_info) %>% as.data.frame()
dim(dem_patients_full) # dim = (25229,11)
```

## 3. Left join find overlap
```{r}
patient_eligible = epi_patients_full %>% left_join(dem_patients_full) %>% 
  filter(record_length > 0 & n_encounter >= 2)
dim(patient_eligible) # dim = (25018,14)
```
...
dim(stroke_case_info) # dim = (608,4)
```
...
dim(hyperlipid_case_info) # dim = (21101,4)
```

### 5) AD
```{r}
ad_case_info = enc_phecode_map_first %>% 
  filter(phecode %in% c("290.11")) %>% group_by(UniqueSampleId) %>% 
  mutate(ad_age_diagnosis = min(age_diagnosis),
         ADDate = min(StartDate)) %>% 
  select(PatientID, UniqueSampleId, ADDate, ad_age_diagnosis) %>% unique()
dim(ad_case_info) # dim = (483,4)
```
...
patient_final = patient_eligible %>% 
  ...
  left_join(ad_case_info) %>% filter(!is.na(epilepsy)) %>% 
  ...
  select(UniqueSampleId, female, gen_ancestry, age_last_visit, 
         ...
         ADDate, ad_age_diagnosis)
dim(patient_final) # dim = (25018,24)
save(patient_final, file = paste0(raw_data_path, 
                                  "modeling/patient_final_04112023.rda"))

patient_final_EUR = patient_final %>% 
  left_join(apoe) %>% left_join(eur_pcs) %>% 
  filter(gen_ancestry == "EUR") %>% 
  filter(age_last_visit >= 60)
dim(patient_final_EUR) # dim = (18742,47)
save(patient_final_EUR, file = paste0(raw_data_path, 
                                      "modeling/EUR/patient_final_EUR_04112023.rda"))
```
...
geno_risk_loci = read.table(file = paste0(FUMA_data_path, "GenomicRiskLoci.txt"), 
                            header = T, sep = "\t", fill = T)
candidate_snps = read.table(file = paste0(FUMA_data_path, "snps.txt"), 
                            header = T, sep = "\t", fill = T)
candidate_snps_short = candidate_snps %>% 
  dplyr::rename("a0" = "non_effect_allele", "a1" = "effect_allele") %>% 
  select(rsID, chr, pos, a0, a1, gwasP, beta, se, CADD)
candidate_indsig_info = candidate_snps %>% 
  mutate(mapping_num = posMapFilt + eqtlMapFilt + ciMapFilt) %>% 
  select(rsID, r2, IndSigSNP, CADD, GenomicLocus, mapping_num)
```
...
candidate_hg19 = candidate_snps_short %>% 
  mutate(No = seq(1,nrow(candidate_snps_short)))
...
dim(candidate_snps_liftover) # dim = 2271,10
...
name_out_AD_Kunkle_EUR = c(name1_out, name2_out) %>% unique()
length(name_out_AD_Kunkle_EUR) # length = 4542
# Write output for snp extraction on server
# write.table(name_out_AD_Kunkle_EUR, 
#             file = paste0(raw_data_path, "sumstats/AD_Kunkle_EUR_wdup.txt"),
#             sep = "\t", quote = F, row.names = F, col.names = F)
```

```{r}
# Read in the QCed genotype and sumstats files + preprocess the bed file (only need to do once for each data set)
geno_file = paste0(raw_data_path, 'atlas/geno/AD.indsig.EUR')
...
rsid_map_beta_align_AD_EUR = snp_to_extract %>% 
  select(rsID, chr, pos, a1, a0, beta, se, gwasP, CADD) %>% unique()
info_snp_AD_EUR = snp_match(rsid_map_beta_align_AD_EUR, map) # 1576 variants have been matched
```

## 2. Select mapped SNPs
```{r}
mapped_snp = info_snp_AD_EUR %>% 
  left_join(candidate_indsig_info) %>% 
  filter(mapping_num > 0 & !is.na(beta)) %>% 
...
# Check how many mapped SNPs are IndSigSNP (N = 92)
...
dim(genotype_df) # 34463,1831
rownames(genotype_df) = obj.bigSNP$fam$sample.ID
```


```{r}
# Perform final matching using selected CADD-1st
final2map = mapped_snp %>% filter(rank_CADD == 1) %>% 
  select(rsID, chr, pos, a1, a0, beta, se, gwasP, CADD, GenomicLocus) %>% unique()
final_map = snp_match(final2map, map) # 143 variants have been matched
```

## 3. Finalize genome dataset
```{r}
genotype_df_map = genotype_df %>% select(all_of(final_map$uniqueID))
dim(genotype_df_map) # 34463,143
...
dim(freeze60k_geno_freq) # dim = c(18742, 189)
