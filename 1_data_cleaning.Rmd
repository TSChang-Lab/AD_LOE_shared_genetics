...
### 1) AD phenotype
```{r}
candidate_snps_AD = read.table(file = paste0(FUMA_data_path, "FUMA_AD_EUR/snps.txt"), 
                               header = T, sep = "\t", fill = T)
candidate_snps_short_AD = candidate_snps_AD %>% 
  ...
  select(rsID, chr, pos, a0, a1, gwasP, beta, se, CADD,
         r2, IndSigSNP, GenomicLocus) %>% 
  mutate(phenotype = "AD")
dim(candidate_snps_short_AD) # dim = (1984,13)
```

### 2) Epilepsy phenotype
```{r}
candidate_snps_epilepsy = read.table(file = paste0(FUMA_data_path, "FUMA_focal_HS/snps.txt"), 
                                     header = T, sep = "\t", fill = T)
...
dim(candidate_snps_short_epilepsy) # dim = (40,13)
```
...
dim(candidate_snps_liftover) # dim = 2013,14
...
length(name_out_full) # length = 4700
...
info_snp = snp_match(rsid_map_beta_align, map) # 1396 variants have been matched
```
...
final2map = mapped_snp %>% filter(rank_CADD == 1) %>% 
  select(rsID, chr, pos, a1, a0, beta, se, gwasP, 
         phenotype, CADD, GenomicLocus) %>% unique()
final_map = snp_match(final2map, map) # 144 variants have been matched
```

## 3. Finalize genome dataset
```{r}
genotype_df_map = genotype_df %>% select(all_of(final_map$uniqueID))
dim(genotype_df_map) # 34463,144
...
dim(freeze60k_geno_freq) # dim = c(17306, 193)
