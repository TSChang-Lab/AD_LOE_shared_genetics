...
candidate_snps_short_AD = candidate_snps_AD %>% 
  ...
  mutate(phenotype = "AD")
dim(candidate_snps_short_AD) # dim = (1984,11)
```
...
dim(candidate_snps_short_epilepsy) # dim = (575,11)
```
...
dim(candidate_snps_liftover) # dim = 2548,12
...
info_snp = snp_match(rsid_map_beta_align, map) # 1804 variants have been matched
```
...
# Check how many mapped SNPs are IndSigSNP (N = 98)
...
final2map = mapped_snp %>% filter(rank_CADD == 1) %>% 
  ...
  dplyr::rename("beta" = "CADD") %>% unique()
final_map = snp_match(final2map, map) # 157 variants have been matched
```
...
dim(freeze60k_geno_freq) # dim = c(17306, 206)
